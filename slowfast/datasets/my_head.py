...
from slowfast.models.losses import *
...
class SlowFastPhraseMMHead(nn.Module):
    def __init__(
        self,
        dim_in=2048,
        dim_hidden=768,
        ...
        super(SlowFastPhraseMMHead, self).__init__()

        self.mm_encoder = CHR_Transformer_3(
            input_dim_q=dim_in,
            num_layers=num_layers,
            model_dim=dim_hidden,
            num_heads=num_head,
            ffn_dim=dim_hidden,
            output_dim=dim_out
        )
        self.itm_head = nn.Sequential(
            nn.Linear(dim_out, 64, bias=True),
    ...
    def forward(self, fpv_feats, tpv_feats, label_matrix):
        # init dim of tokens: (N, C, T, H, W) -> (N, C, O)
        bs, c = fpv_feats.size()[0], fpv_feats.size()[1]
        fpv_tokens = fpv_feats.view(bs, c, -1)
        tpv_tokens = tpv_feats.view(bs, c, -1)
        fpv_tokens_norm = F.normalize(fpv_tokens, dim=1)
        tpv_tokens_norm = F.normalize(tpv_tokens, dim=1)

        output_tokens = self.mm_encoder(tpv_tokens_norm.permute((0, 2, 1)), fpv_tokens_norm)
        avg_token = F.adaptive_avg_pool1d(output_tokens.permute((0, 2, 1)), (1)).squeeze()
        avg_token_norm = F.normalize(avg_token, dim=-1)
        itm_preds = self.itm_head(avg_token_norm).squeeze()
        itm_loss = self.itm_loss(itm_preds, self.cos2preb(torch.diagonal(label_matrix)))
        print(output_tokens.permute((0, 2, 1)).size(), avg_token.size())
        return itm_loss
...
class SlowFastCHRHead(nn.Module):
    def __init__(
        self,
        dim_in,
        dim_out,
        ...
        super(SlowFastCHRHead, self).__init__()

        self.num_token = cfg.MODEL.EPIC_TOKEN_V
        self.head_cam_fpv = nn.Linear(sum(dim_in), dim_out, bias=True)
        self.head_cam_tpv = nn.Linear(sum(dim_in), dim_out, bias=True)
        self.slow_only = slow_only
        self.temp_st = cfg.MODEL.TEMP_CHR
        self.chr_trans = CHR_Transformer_3(
            input_dim_q=cfg.CHR_TRANS.ID,
            num_layers=cfg.CHR_TRANS.N_LAYER,
            model_dim=cfg.CHR_TRANS.HD,
            num_heads=cfg.CHR_TRANS.N_HEAD,
            ffn_dim=cfg.CHR_TRANS.HD,
            output_dim=cfg.CHR_TRANS.OD
        )
    ...
    def cam_query_vn_y(self, x_ori, pred_proj_v, pred_proj_n, n_token, labels_v, labels_n):
        """
            Qitong on Jul. 25th, 2022.
        """
        bs, c, t, h, w = x_ori[0].size()
        ...
        res_tensor_list = []
        idxs_list = []
        for cf_v, cf_n, st_x, y_v, y_n in zip(cls_features_v, cls_features_n, x_ori[0], labels_v, labels_n):
            st_x_ = st_x.view(c, -1)
            values_v_map_final_norm = (cf_v[y_v.item()] - torch.min(cf_v[y_v.item()])) / \
                                      (torch.max(cf_v[y_v.item()]) - torch.min(cf_v[y_v.item()]))
            values_n_map_final_norm = (cf_n[y_n.item()] - torch.min(cf_n[y_n.item()])) / \
                                      (torch.max(cf_n[y_n.item()]) - torch.min(cf_n[y_n.item()]))
            values, idxs = (values_v_map_final_norm + values_n_map_final_norm).topk(n_token)
            # res_tensor_list.append(((values / 2) * st_x_[:, idxs]).permute((1, 0)))
            res_tensor_list.append(st_x_[:, idxs].permute((1, 0)))
            idxs_list.append(idxs)
        return torch.stack(res_tensor_list, dim=0)

    def cam_query_vn_y_plus(self, x_ori, pred_proj_v, pred_proj_n, n_token, labels_v, labels_n):
        """
            Qitong on Jul. 25th, 2022.
        """
        bs, c, t, h, w = x_ori[0].size()
        ...
        res_tensor_list = []
        idxs_list = []
        for cf_v, cf_n, st_x, l_v, l_n in zip(cls_features_v, cls_features_n, x_ori[0], labels_v, labels_n):
            y_v = torch.nonzero(l_v)
            y_n = torch.nonzero(l_n)
            st_x_ = st_x.view(c, -1)
            v_list, n_list = [], []
            for i in y_v:
                values_v_map_final_norm = (cf_v[i.item()] - torch.min(cf_v[i.item()])) / \
                                          (torch.max(cf_v[i.item()]) - torch.min(cf_v[i.item()]))
                v_list.append(values_v_map_final_norm)
            for i in y_n:
                values_n_map_final_norm = (cf_n[i.item()] - torch.min(cf_n[i.item()])) / \
                                          (torch.max(cf_n[i.item()]) - torch.min(cf_n[i.item()]))
                n_list.append(values_n_map_final_norm)
            if len(v_list) > 0 and len(n_list) > 0:
                values_v_map_final, _ = torch.max(torch.stack(v_list, dim=0), dim=0)
                values_n_map_final, _ = torch.max(torch.stack(n_list, dim=0), dim=0)
                values, idxs = (values_v_map_final + values_n_map_final).topk(n_token)
            elif len(v_list) > 0:
                values_v_map_final, _ = torch.max(torch.stack(v_list, dim=0), dim=0)
                # values_n_map_final, _ = torch.max(torch.stack(n_list, dim=0), dim=0)
                values, idxs = values_v_map_final.topk(n_token)
            elif len(n_list) > 0:
                # values_v_map_final, _ = torch.max(torch.stack(v_list, dim=0), dim=0)
                values_n_map_final, _ = torch.max(torch.stack(n_list, dim=0), dim=0)
                values, idxs = values_n_map_final.topk(n_token)
            else:
                raise RuntimeError("So sad!!! :(")
            res_tensor_list.append(st_x_[:, idxs].permute((1, 0)))
            idxs_list.append(idxs)
        return torch.stack(res_tensor_list, dim=0)

    def _get_chr_multilabel_kl_loss_4(self, feat_hj, feat_hi, feat_zi,
                                      label_v, label_lt_v, label_n, label_lt_n, same_idx):
        """
            Qitong on Aug. 7th, 2022.
        """
        sim_labels = (label_v @ label_lt_v.T + label_n @ label_lt_n.T) / 2
        ...
        for i in range(sim_targets.size()[0]):
            if same_idx[i].item() == 2:
                ...
                raise NotImplementedError
        sim_preds = torch.zeros(sim_targets.size()).to(sim_targets.device)
        for i in range(feat_hj.size()[0]):
            for j in range(feat_hi.size()[0]):
                w_idxs = torch.max(feat_hj[i] @ feat_hi[j].T / self.temp_st, dim=0).indices
                sim_preds[i][j] = torch.diagonal(feat_hj[i] @ feat_zi[j][w_idxs, :].T / self.temp_st, 0).mean()
        loss_kl = -torch.sum(F.log_softmax(sim_preds, dim=1) * sim_targets, dim=1).mean()
        return loss_kl

    def forward(self, feats_fpv, feats_tpv, cls_head,
                labels_fpv, labels_tpv_v, labels_tpv_n,
                labels_norm_verb, labels_norm_noun,
                labels_norm_verb_lt, labels_norm_noun_lt, same_idx):
        fpv_cam_tokens = self.cam_query_vn_y(self.combine_slow_fast(feats_fpv[0], feats_fpv[1]),
                                             cls_head.projections[0], cls_head.projections[1],
                                             n_token=self.num_token,
                                             labels_v=labels_fpv[:, 0], labels_n=labels_fpv[:, 1])
        tpv_cam_tokens = self.cam_query_vn_y_plus(self.combine_slow_fast(feats_tpv[0], feats_tpv[1]),
                                                  cls_head.projections[0], cls_head.projections[1],
                                                  n_token=self.num_token,
                                                  labels_v=labels_tpv_v, labels_n=labels_tpv_n)
        fpv_cam_tokens_pre_norm = self.head_cam_fpv(fpv_cam_tokens)
        fpv_cam_tokens_norm = F.normalize(fpv_cam_tokens_pre_norm, dim=2)
        tpv_cam_tokens_pre_norm = self.head_cam_tpv(tpv_cam_tokens)
        tpv_cam_tokens_norm = F.normalize(tpv_cam_tokens_pre_norm, dim=2)
        if self.slow_only:
            bs, c = feats_tpv[0].size()[0], feats_tpv[0].size()[1]
            tpv_kv_tokens_norm = F.normalize(feats_tpv[0].view(bs, c, -1), dim=1)
        else:
            raise NotImplementedError("TODO List of CHR...")
        after_tpv_pre_norm = self.chr_trans(fpv_cam_tokens_norm, tpv_kv_tokens_norm)
        after_tpv_norm = F.normalize(after_tpv_pre_norm, dim=2)
        loss_kl_f2t = self._get_chr_multilabel_kl_loss_4(
            feat_hj=tpv_cam_tokens_norm,
            feat_hi=fpv_cam_tokens_norm,
            feat_zi=after_tpv_norm,
            label_v=labels_norm_verb,
            label_lt_v=labels_norm_verb_lt,
            label_n=labels_norm_noun,
            label_lt_n=labels_norm_noun_lt,
            same_idx=same_idx,
        )
        return loss_kl_f2t