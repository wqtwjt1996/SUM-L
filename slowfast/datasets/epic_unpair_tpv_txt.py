...
class Epickitchen_lemma_txt(torch.utils.data.Dataset):
    '''
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        if self.mode == "train":
            ...
            self.verb_dict, self.noun_dict = data_utils.get_lemma_to_epic55_labels()
        ...
        if self.mode == "train" and self.lemma_dict_bool:
            ...
            with open('./pesudo_ep55_data_0817.json', 'w') as json_file:
                json_file.write(json_str)
            print("Done!!")
            exit(32)
        else:
            with open("./pesudo_ep55_data_0817.json", 'r') as load_f:
                self.load_dict = json.load(load_f)
            self.dict_txt = self._load_one_pair()
            logger.info('Loading pseudo pair dictionary finished! (0817)')

    def _load_one_pair(self):
        f = open("0818_txt_pos_mining_2.txt", "r")
        dict_txt = {}
    ...
    def _forward_lemma(self, index):
        ...
        verb_dict, noun_dict = utils.get_lemma_to_epic55_labels()
