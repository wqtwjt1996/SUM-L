...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        epoch = -1
    elif "EgoVLP" in path_to_checkpoint:
        ...
        # print(checkpoint["model_state"]["head.projection.weight"].size(),
        #       checkpoint["model_state"]["head.projection.bias"].size(), 233)
        for key in keys:
            if 'module.video_model.' in key:
                newkey = key.replace('module.video_model.', '')
                checkpoint["state_dict"][newkey] = checkpoint["state_dict"][key]
                del checkpoint["state_dict"][key]
        ms.load_state_dict(checkpoint["state_dict"], strict=False)
        noinit = []
