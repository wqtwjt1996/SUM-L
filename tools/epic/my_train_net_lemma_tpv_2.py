...
phrase_model = SentenceTransformer('sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2')

for p in phrase_model.parameters():
...
def train_epoch(
    ...
    if cfg.MODEL.FREEZE_STAGES > 0:
        for name, m in model.named_modules():
            if isinstance(m, torch.nn.BatchNorm3d) and name.startswith("module.s") and int(
                    name[8]) <= cfg.MODEL.FREEZE_STAGES \
                    and name[9] == ".":
                m.eval()
                logger.info(f"freeze bn layer {name}")

    for cur_iter, (inputs, tpv_imgs, labels, index, meta, path_fpv) in enumerate(train_loader):
        ...
        if isinstance(tpv_imgs, (list,)):
            for i in range(len(inputs)):
                tpv_imgs[i] = tpv_imgs[i].cuda(non_blocking=True)
        else:
            tpv_imgs = tpv_imgs.cuda(non_blocking=True)
        labels = labels.cuda()
        labels_verb_vec = meta["verb_vec"].cuda(non_blocking=True)
        labels_noun_vec = meta["noun_vec"].cuda(non_blocking=True)

        # Update the learning rate.
        lr = optim.get_epoch_lr(cur_epoch + float(cur_iter) / data_size, cfg)
        optim.set_lr(optimizer, lr)

        preds, addi_loss = model(inputs, tpv_imgs, cur_epoch=cur_epoch,
                                 labels=labels, labels_verb=labels_verb_vec, labels_noun=labels_noun_vec,
                                 same_idx=meta["same_idx"], meta=meta, phrase_model=phrase_model)
        ...
        for name, param in model.named_parameters():
            if param.grad is None and not ("_m" in name or "_k400" in name):
                print(name)
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, _, labels, _, meta, _) in enumerate(val_loader):
